...
const AdminOrdersPage: React.FC = () => {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiClient.get(
        `/admin/restaurants/${restaurantId}/orders?${params.toString()}`
      );
  ...
  const handleUpdateOrderStatus = async (
    ...
    try {
      await apiClient.put(
        `/admin/restaurants/${restaurantId}/orders/${orderId}/status`,
        { status: newStatus }
      );
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex-grow">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10 px-4 bg-red-50 text-red-700 rounded-md border border-red-200">
            ...
            <button
              onClick={fetchOrders}
              className="mt-4 px-4 py-1.5 border border-red-300 rounded text-sm hover:bg-red-100"
            >
              Retry
            </button>
          </div>
