...
export const GET = withStaffAuth<OrderResponse[] | {}>(
  async (request, context, user) => {
    ...
    console.log(
      `GET /orders: User ${
        user.email
      } fetching orders for restaurant ${restaurantId} with status ${
        status || "any"
      }`
    );
    ...
    try {
      ...
      if (status && status.length > 0) {
        ...
        if (status.length > 30) {
          return NextResponse.json(
            { message: "Too many status values provided for filter (max 30)." },
            { status: 400 }
          );
        }
      ...
      console.error(
        `GET /orders: Error fetching orders for restaurant ${restaurantId}:`,
        error
      );
      ...
      if (
        ...
        console.error(
          "Firestore index missing! Check the error details in the Firebase console or logs to create the required index (e.g., for status + createdAt)."
        );
        return NextResponse.json(
          {
            message:
              "Internal Server Error: Database configuration issue (index missing).",
            error: error.message,
          },
          { status: 500 }
        );
