...
export const PUT = withStaffAuth<UpdateInfoResponse>(
  async (request, context, user) => {
    ...
    console.log(
      `PUT /info: Staff ${staffUserId} updating info/content for restaurant ${restaurantId}`
    );
    ...
    try {
      ...
      if (body.info && typeof body.info === "object") {
        ...
        if (typeof info.name === "string" && info.name.trim())
          updateData["info.name"] = info.name.trim();
        // Allow empty strings for optional fields like description, location
        if (typeof info.description === "string")
          updateData["info.description"] = info.description;
        if (typeof info.location === "string")
          updateData["info.location"] = info.location.trim(); // Trim URLs
        if (typeof info.OpeningTime === "string")
          updateData["info.OpeningTime"] = info.OpeningTime;

        // Contact sub-object: only update if fields are present and non-empty
        if (info.contact && typeof info.contact === "object") {
          if (
            typeof info.contact.email === "string" &&
            info.contact.email.trim()
          )
            updateData["info.contact.email"] = info.contact.email.trim();
          if (
            typeof info.contact.phone === "string" &&
            info.contact.phone.trim()
          )
            updateData["info.contact.phone"] = info.contact.phone.trim();
        }
        // Opening Hours array: Overwrite only if a valid array is provided
        if (Array.isArray(info.openingHours)) {
          const validHours = info.openingHours.filter(
            (h) =>
              h &&
              typeof h.day === "string" &&
              h.day.trim() !== "" &&
              typeof h.timing === "string" &&
              h.timing.trim() !== ""
          );
        ...
        if (
          info.social &&
          typeof info.social === "object" &&
          !Array.isArray(info.social)
        ) {
          const socialUpdate: { [key: string]: string } = {};
          if (typeof info.social.facebook === "string")
            socialUpdate.facebook = info.social.facebook.trim();
          if (typeof info.social.instagram === "string")
            socialUpdate.instagram = info.social.instagram.trim();
          // Add other platforms if needed
          updateData["info.social"] = socialUpdate;
        }
      }
      ...
      if (body.siteContent && typeof body.siteContent === "object") {
        ...
        if (typeof content.heroText === "string")
          updateData["siteContent.heroText"] = content.heroText;
        if (typeof content.heroSubtext === "string")
          updateData["siteContent.heroSubtext"] = content.heroSubtext;
        if (typeof content.menuText === "string")
          updateData["siteContent.menuText"] = content.menuText;
        if (typeof content.menuSubtext === "string")
          updateData["siteContent.menuSubtext"] = content.menuSubtext;
        ...
        if (Array.isArray(content.featuring)) {
          const validFeatures = content.featuring
            .filter(
              (f) =>
                f &&
                (f.title?.trim() || f.description?.trim() || f.imageUrl?.trim())
            ) // Keep if any field has value
            .map((f) => ({
              // Ensure only expected fields are saved, trim values
              title: f.title?.trim() || "",
      ...
      if (Object.keys(updateData).length === 0) {
        return NextResponse.json(
          { success: true, message: "No valid changes detected to save." },
          { status: 200 }
        );
      }
      ...
      console.log(
        `Info/Content updated successfully for restaurant ${restaurantId}.`
      );
      ...
      return NextResponse.json(
        {
          success: true,
          message:
            "Restaurant information and site content updated successfully.",
        },
        { status: 200 }
      );
    } catch (error: any) {
      console.error(
        `PUT /info: Error updating info/content for restaurant ${restaurantId}:`,
        error
      );
