...
const handler = async (
  ...
  console.log(
    `GET /api/orders/mine/active: Fetching ACTIVE orders for user ${userId}`
  );
  ...
  try {
    ...
    console.log(
      `GET /api/orders/mine/active: Found ${orders.length} active orders for user ${userId}.`
    );

    return NextResponse.json(orders, { status: 200 });
  } catch (error: any) {
    console.error(
      `GET /api/orders/mine/active: Error fetching active orders for user ${userId}:`,
      error
    );
    if (
      ...
      console.error(
        "Firestore index missing for active orders query! Needs index on 'userId' (asc), 'status' (in), and 'createdAt' (desc) for collection group 'orders'."
      );
    ...
    return NextResponse.json(
      { message: "Failed to fetch active orders.", error: error.message },
      { status: 500 }
    );
  }
};
