...
import type {
  ...
  Address,
  CartItemOptions,
} from "@/constants/types"; // Adjust path
...
> => {
  ...
  try {
    ...
    console.log(
      `Initiating backend checkout for user: ${userId}, restaurant: ${restaurantId}`
    );
    ...
    for (const item of cartItems) {
      try {
        const unitAmount = await calculateBackendPriceForItem(
          restaurantId,
          item
        );
        ...
        cartItems: JSON.stringify(
          cartItems.map((i) => ({
            ...
            unitPrice:
              (line_items.find(
                (li) =>
                  li.price_data?.product_data?.metadata?.menuItemId === i.id
              )?.price_data?.unit_amount ?? 0) / 100, // Store calculated unit price
          }))
        ),
      },
    });
