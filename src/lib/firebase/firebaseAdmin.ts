...
try {
    ...
    if (!serviceAccountEnvJson || serviceAccountEnvJson.trim() === '' || serviceAccountEnvJson.trim() === '{}') {
        console.error(`[firebaseAdmin-BUILD-DEBUG] CRITICAL for ${ADMIN_APP_NAME}: GRILL_SHACK_ADMIN_SDK_JSON is not set or is empty.`);
        throw new Error(`GRILL_SHACK_ADMIN_SDK_JSON is required for ${ADMIN_APP_NAME}.`);
    }
    ...
    console.error(`[firebaseAdmin-BUILD-DEBUG] CRITICAL ERROR for ${ADMIN_APP_NAME}:`, e.constructor.name, e.message);
    if (typeof app !== 'undefined' && app && app.name) {
         console.error(`[firebaseAdmin-BUILD-DEBUG] App object state at error for <span class="math-inline">\{ADMIN\_APP\_NAME\}\: name\=</span>{app.name}`);
    } else {
         console.error(`[firebaseAdmin-BUILD-DEBUG] App object for ${ADMIN_APP_NAME} was not successfully initialized or available at point of error.`);
    }
    console.error(`[firebaseAdmin-BUILD-DEBUG] For ${ADMIN_APP_NAME}, typeof admin.firestore: ${typeof admin.firestore}`);
    console.error(`[firebaseAdmin-BUILD-DEBUG] For ${ADMIN_APP_NAME}, typeof admin.auth: ${typeof admin.auth}`);
    console.error(`[firebaseAdmin-BUILD-DEBUG] Error Stack for ${ADMIN_APP_NAME}:`, e.stack);
    throw e; 
}
...
export const adminAuth = (global as any)[`${ADMIN_APP_NAME}_auth`];
