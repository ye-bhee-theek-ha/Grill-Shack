  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (!Array.isArray(response.data)) {
        console.error(
          "API Error: /api/orders/mine did not return an array.",
          response.data
        );
...
export const fetchActiveUserOrders = createAsyncThunk<Order[]>(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      if (!Array.isArray(response.data)) {
        console.error(
          "API Error: /api/orders/mine/active did not return an array.",
          response.data
        );
    ...
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<Order[]>) => {
          ...
          state.allOrders = action.payload.sort(
            (a, b) =>
      ...
      .addCase(
        fetchActiveUserOrders.fulfilled,
        (state, action: PayloadAction<Order[]>) => {
          state.loadingActive = "succeeded";
          // Sort orders by creation date, newest first
          state.activeOrders = action.payload.sort(
            (a, b) =>
              (b.createdAt instanceof Date
                ? b.createdAt.getTime()
                : new Date(b.createdAt as string).getTime()) -
...
// Export actions
export const { updateOrderStatus, clearOrderErrors, addOrderLocally } =
  orderSlice.actions;

// --- Selectors ---
export const selectUserAllOrders = (state: RootState): Order[] =>
  state.orders.allOrders;
export const selectUserActiveOrders = (state: RootState): Order[] =>
  state.orders.activeOrders;
export const selectOrdersLoadingAll = (state: RootState): boolean =>
  state.orders.loadingAll === "pending";
export const selectOrdersLoadingActive = (state: RootState): boolean =>
  state.orders.loadingActive === "pending";
export const selectOrdersErrorAll = (state: RootState): string | null =>
  state.orders.errorAll;
export const selectOrdersErrorActive = (state: RootState): string | null =>
  state.orders.errorActive;
