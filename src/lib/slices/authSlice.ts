  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      await signOut(auth).catch((err) =>
        console.error("Client signout failed during logout error:", err)
      );
      clearRecaptchaVerifier();
...
export const registerUser = createAsyncThunk<
  ...
  RegisterPayload
>("auth/registerUserApi", async (registerData, { rejectWithValue }) => {
  try {
  ...
  async (
    ...
    if (!currentUser) {
      return rejectWithValue({
        message: "User must be logged in client-side to verify phone.",
      });
    }
    ...
    try {
      ...
      try {
        ...
        return rejectWithValue({
          message: `Failed to get authentication token: ${tokenError.message}`,
        });
      }
      ...
      try {
        ...
        try {
          ...
          console.log(
            "Session login initiated successfully after phone verification."
          );
          ...
          console.error(
            "Auto-login step failed after phone verification:",
            loginError
          );
          ...
          return rejectWithValue({
            message: `Phone verified, but auto-login failed: ${
        ...
        console.error(
          "Backend update failed after phone linking:",
          backendError
        );
        clearRecaptchaVerifier(); // Clear verifier on backend error
        return rejectWithValue({
          message: `Phone linked, but backend update failed: ${
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        sendPhoneVerification.fulfilled,
