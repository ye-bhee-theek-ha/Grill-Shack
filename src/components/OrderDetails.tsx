...
export const OrderOptionDisplay: React.FC<{
  options: Record<string, any> | undefined;
  classname?: string;
}> = ({ options, classname = "text-white/80" }) => {
  ...
  const displayOptions = Object.entries(options)
    .map(([key, value]) => {
      ...
      if (Array.isArray(value)) {
        ...
        if (key === "_additionalNote" && value.trim() === "") return null;
        else if (key === "_additionalNote") {
          displayValue = "Additional Note: " + value;
        } else {
  ...
  return (
    <div className={`text-normal5 ${classname} mt-0.5 flex flex-col gap-0.5`}>
...
const ChefIcon = () => (
  ...
  >
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a2 2 0 01-2-2V10a2 2 0 012-2h2V6l4 2zM7 8V6l4-2v4"
    />
  </svg>
);
const BagIcon = () => (
...
const MapPinIcon = () => (
  <svg
    ...
    className="h-4 w-4 mr-1.5 text-gray-400 flex-shrink-0"
    fill="none"
    viewBox="0 0 24 24"
    stroke="currentColor"
  >
...
const OrderDetails: React.FC = () => {
  ...
  if (isLoading && !order) {
    // Show loading only if no order data is available yet
    return (
  ...
  const estimatedTime = order.estimatedCompletionTime
    ? new Date(order.estimatedCompletionTime as string).toLocaleTimeString([], {
        hour: "numeric",
        minute: "2-digit",
      })
    : null;
  ...
  return (
    <div className="bg-white/10 p-4 sm:p-6 rounded-lg shadow-md border border-gray-100/20 w-full max-w-2xl mx-auto my-6">
      {/* Order Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 pb-4 border-b border-white border-dashed">
        <div>
        ...
        <div
          className={`mt-2 sm:mt-0 inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${bgColorClass} ${colorClass}`}
        >
      ...
      {/* Progress Bar */}
      <div className="w-full bg-gray-200/75 rounded-full h-2.5 mb-4 dark:bg-gray-700">
        <motion.div
      ...
      <p className="text-normal4 text-white/70 mb-3">
        Placed at:{" "}
        {order.createdAt
      ...
      {/* Estimated Time */}
      {estimatedTime && (
        <p className="text-center text-normal4 text-white/70 mb-4">
          Estimated Delivery Time:{" "}
          <span className="font-medium text-primary-dark">{estimatedTime}</span>
        </p>
      )}
      ...
      {/* Totals */}
      <div className="text-sm border-t border-white border-dashed pt-3 mt-3 space-y-1">
