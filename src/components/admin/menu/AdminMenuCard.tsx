...
import placeholderMenuImgSrc from '@/../public/Images/menu2.png'; // Renamed for clarity
...
const AdminMenuCard: React.FC<AdminMenuCardProps> = ({ item, onEdit, onDelete }) => {
    ...
    const imageSrc = item.imageUrl || placeholderMenuImgSrc;
    const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
        // Type assertion for currentTarget
        (e.currentTarget as HTMLImageElement).src = placeholderMenuImgSrc.src;
    };
    ...
    return (
        <div className="bg-neutral-800 rounded-lg shadow-lg border border-neutral-700/80 overflow-hidden flex flex-col h-full transition-all duration-300 hover:shadow-xl">
            ...
            <div className="relative h-40 w-full bg-neutral-700">
                ...
                />
                {/* Availability Badge */}
                <span className={`absolute top-2.5 left-2.5 px-2.5 py-1 rounded-full text-xs font-semibold shadow ${item.isAvailable === false ? 'bg-red-600 text-white' : 'bg-emerald-600 text-white'}`}>
            ...
            <div className="p-4 flex flex-col flex-grow">
                <h3 className="text-lg font-semibold text-neutral-100 truncate mb-1.5">{item.name || "Unnamed Item"}</h3>
                <p className="text-sm text-neutral-400 line-clamp-2 mb-3 flex-grow min-h-[2.5em]">{item.description || 'No description available.'}</p>

                {/* Price & Category */}
                <div className="flex justify-between items-center text-sm mb-3">
                    <span className="font-bold text-lg text-[#9A080F]">{formatCurrency(item.price)}</span>
                    <span className="text-xs bg-neutral-700 text-neutral-300 px-2 py-1 rounded-md">{item.categoryId || 'Uncategorized'}</span>
                </div>
                ...
                {item.tags && item.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                        {item.tags.slice(0, 3).map((tag, idx) => (
                            <span key={idx} className="text-[10px] px-2 py-0.5 bg-[#B41219]/20 text-[#B41219] border border-[#B41219]/50 rounded-full font-medium">
                                {tag}
                            </span>
                        ))}
                        {item.tags.length > 3 && (
                ...
                <div className="flex justify-end items-center gap-2 mt-auto border-t border-neutral-700 pt-3">
                    ...
                    >
                        {isDeleting ? <Spinner className="w-4 h-4" /> : <DeleteIcon className="w-4 h-4" />}
                    </button>
                </div>
            </div>
        </div>
