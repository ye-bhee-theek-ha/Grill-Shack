...
import { OrderOptionDisplay } from "../OrderDetails";
...
const formatDateTime = (
  dateInput: Date | string | Timestamp | undefined | null
): string => {
  ...
  try {
    ...
    if (
      ...
      date = new Timestamp(
        (dateInput as any)._seconds,
        (dateInput as any)._nanoseconds
      ).toDate();
    } else {
      console.warn(
        "formatDateTime received unexpected type:",
        typeof dateInput,
        dateInput
      );
    ...
    if (isNaN(date.getTime())) {
      console.warn(
        "formatDateTime resulted in an invalid date from input:",
        dateInput
      );
      return "Invalid Date";
    }
  ...
  return (
    <div className="bg-white/5 rounded-lg shadow border border-gray-100/15 overflow-hidden">
      ...
      <div className="px-4 py-4 sm:px-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-1">
          <>
            ...
            <p className="text-sm text-white">{order.name}</p>
        ...
        <div className="md:col-span-2">
          ...
          <div className="flex justify-end text-white font-semibold text-normal2 mt-2 pt-1 border-t">
      ...
      {possibleNextStatuses.length > 0 && ( // Only show actions if there are next steps
        <div className="px-4 py-3 sm:px-6 bg-white/20 border-t border-white/40">
          <div className="flex flex-col sm:flex-row items-center gap-3">
            ...
            <select
              ...
              onChange={(e) =>
                setSelectedNextStatus(e.target.value as OrderStatus)
              }
              disabled={isUpdating}
