...
const TrashIcon = () => (
  <svg
    ...
    height="16"
    fill="currentColor"
    className="bi bi-trash3 text-gray-500 hover:text-primary"
    viewBox="0 0 16 16"
  >
  ...
  return (
    <div className="flex flex-wrap gap-1 mt-1">
      {displayOptions.map((optionText, index) => (
        <span
          key={index}
          className="text-[10px] text-grey bg-white/20 px-1.5 py-0.5 rounded"
        >
          {optionText}
        </span>
      ))}
...
const CartModal: React.FC<{
...
}> = ({ OnLoginRequired, onProceedToConfirmation }) => {
  ...
  const handleCheckout = () => {
    ...
    if (!profile?.phoneNumber && !profile?.phoneVerified) {
      ...
      setCheckoutError(
        "Please add and verify your phone number in your profile."
      );
      return;
    }
  ...
  if (!items || items.length === 0) {
    ...
    return (
      <div className="absolute top-full right-0 mt-2 w-80 md:w-96 bg-neutral-800 rounded-lg shadow-xl z-50 border border-gray-200 p-4 text-center">
        <button
          onClick={handleClose}
  ...
  return (
    <div className="absolute top-full right-0 mt-2 w-80 md:w-96 max-h-[70vh] flex flex-col bg-neutral-800 rounded-lg shadow-xl z-50 border border-primary-dark overflow-hidden">
      {/* Header */}
      <div className="flex justify-between items-center p-4 border-b border-primary-dark">
          ...
          <div key={item.cartItemId} className="flex items-start space-x-3">
            ...
            <div className="flex-grow">
              ...
              <div className="flex items-center mt-1">
                ...
                <span className="mx-2 text-sm font-medium text-white w-4 text-center">
                  {item.quantity}
                </span>
                <button
      ...
      {/* Footer */}
      <div className="p-4 border-t border-primary-dark bg-neutral-800">
        {/* Loyalty Points */}
        ...
        <div className="flex justify-between items-center mb-3">
          <span className="text-md font-semibold text-white">Subtotal</span>
        ...
        {checkoutError && (
          <div className="mb-3 p-2 bg-red-100 text-red-700 text-xs text-center rounded">
            ...
            {checkoutError.includes("phone") && (
              <button
                onClick={() => {
                  router.push("/profile?section=contact");
                  handleClose();
                }}
            ...
            {checkoutError.includes("address") && (
              <button
                onClick={() => {
